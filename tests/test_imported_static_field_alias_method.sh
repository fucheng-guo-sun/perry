...
cat > "$TMPDIR/lib.ts" <<'TS'
class Holder {
  static create = (value: string) => ({ value });
}
const mapType = Holder.create;
export { Holder, mapType as map };
export const z = { map: mapType };
TS

cat > "$TMPDIR/main.ts" <<'TS'
import { Holder, z } from "./lib";
...
echo "PASS: imported static field alias method"
