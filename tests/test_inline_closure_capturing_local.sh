...
FIXTURE="$ROOT/tests/issue_inline_closure_capturing_local.js"
WORKDIR="${TMPDIR:-/tmp}/perry-inline-closure-capture-$$"
...
if [[ "$rc" -ne 0 ]]; then
  echo "inlined-closure-capture fixture failed (exit $rc):" >&2
  cat "$OUT_LOG" >&2
  exit 1
fi

grep -qx "inlined closure captures ok" "$OUT_LOG" || {
