...
/// Issue #740: walk every `Let { name: X, init: Call(FuncRef(f), args) }` in
/// the module and, when `f`'s body is a bare `Return(Some(ClassRef(C)))` AND
/// `C` has `__perry_cap_*` ctor params (i.e. `C` was declared inside `f` and
/// captures outer-scope locals), specialize `C` per call site: clone the
/// class with each capture-param's id bound to the corresponding arg
/// expression (substituted into method/getter/setter/field-init bodies),
/// drop the capture ctor param + matching synthetic field + the ctor body's
/// capture assignment, and rewrite the Let's init to
/// `ClassRef(C_inline_<n>)`. After this pass, the standalone
/// `new <X>()` site can construct the specialized class with no args because
/// the captured value is baked in as a constant in the clone's field
/// initializers / method bodies. Refs #740.
fn specialize_captured_class_factories(module: &mut Module) {
