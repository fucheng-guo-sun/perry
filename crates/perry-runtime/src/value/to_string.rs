...
/// `ToPrimitive(value, "default")` for the binary `+` operator (#3562/#3563).
///
/// Returns a NaN-boxed *primitive* value (string / number / boolean / null /
/// undefined / bigint / symbol). The `+` operator uses the "default" hint,
/// which for `OrdinaryToPrimitive` means method order `valueOf` then
/// `toString` (the reverse of the "string" hint).
///
/// Per the ECMAScript algorithm:
/// - Primitives (non-pointer NaN-box tags) pass through unchanged.
/// - Symbols pass through unchanged — the caller is responsible for the
///   `TypeError` that `Symbol + x` / `x + Symbol` must throw, since the symbol
///   only errors when subsequently coerced to a number or string.
/// - Boxed primitive wrappers (`new Number`/`new String`/`new Boolean`) yield
///   their wrapped payload (#3563).
/// - Dates use the *string* hint (their `[Symbol.toPrimitive]` switches a
///   "default" hint to "string"), so `date + x` concatenates the date string.
/// - Arrays / plain objects / class instances / functions consult
///   `[Symbol.toPrimitive]` first, then fall back to `valueOf`→`toString`.
///   When neither yields a primitive we return the object's string form
///   (`[object Object]`, the array `join(",")`, the function source, etc.),
///   matching `js_jsvalue_to_string`.
pub(crate) unsafe fn to_primitive_for_add(value: f64) -> f64 {
    let jsval = JSValue::from_bits(value.to_bits());
    if !jsval.is_pointer() {
        // Already a primitive (number/int32/string/short-string/bool/null/
        // undefined/bigint live in non-POINTER tags). Return as-is.
        return value;
    }
    ...
    // Symbols: pass through. Detect via side-table before any GC-header read.
    if crate::symbol::is_registered_symbol(ptr) {
        return value;
    }
    // Boxed primitive wrappers: ToPrimitive yields the wrapped primitive
    // directly (#3563).
    if let Some(payload) = crate::builtins::boxed_primitive_value(value) {
        return payload;
    }
    // Dates: the default hint is treated as "string" (Date's own
    // [Symbol.toPrimitive] maps default→string), so `date + x` concatenates.
    ...
    // Custom [Symbol.toPrimitive]("default") wins when present.
    let via_sym = crate::symbol::js_to_primitive(value, 0);
    if via_sym.to_bits() != value.to_bits() {
        // The user method returned something; if it's still an object,
        // continue coercing via the string path below; otherwise it's the
        // primitive result.
        let r = JSValue::from_bits(via_sym.to_bits());
        if !r.is_pointer()
            || crate::symbol::is_registered_symbol((via_sym.to_bits() & POINTER_MASK) as usize)
        {
            return via_sym;
        }
    }
    // OrdinaryToPrimitive(value, "default"): valueOf then toString.
    let scope = crate::gc::RuntimeHandleScope::new();
    let value_handle = scope.root_nanbox_f64(value);
    // Bound recursion via the shared string-hint depth guard.
    let depth = TO_PRIMITIVE_DEPTH.with(|c| c.get());
    if depth < 200 {
        TO_PRIMITIVE_DEPTH.with(|c| c.set(depth + 1));
        let value_of = call_method_for_primitive(&scope, &value_handle, b"valueOf");
        if let MethodOutcome::Primitive(p) = value_of {
            TO_PRIMITIVE_DEPTH.with(|c| c.set(depth));
            return p;
        }
        TO_PRIMITIVE_DEPTH.with(|c| c.set(depth));
    }
    // Fall back to the object's string form (toString / array join / function
    // source / `[object Object]`), exactly as `js_jsvalue_to_string` produces.
